...
# #+RESULTS:
# : [nil 0 1 0 nil require (hydra nil t) idle 0]
...
#+RESULTS:
| (closure (bootstrap-version t) nil (add-hook 'after-save-hook #'efs/org-babel-tangle-config)) | rainbow-mode | evil-org-mode | org-superstar-mode | org-tempo-setup | ob-ipython-auto-configure-kernels | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes | org-appear-mode | org-make-toc-mode | dw/org-mode-setup | dw/org-mode-visual-fill | (closure ((mode . org-mode-hook) (--dolist-tail--) bootstrap-version t) nil (display-line-numbers-mode 0)) |

This takes care of a json-read-table error that pops up on the cluster
#+begin_src emacs-lisp :tangle no

  (defun org-python-use-venv ()
    (when (bound-and-true-p pyvenv-virtual-env)
      (setq-local org-babel-python-command
                  (concat pyvenv-virtual-env "/bin/python"))))

  (advice-add 'org-babel-execute:python :before 'org-python-use-venv)

  (defun reset-python-session ()
    (when (eq major-mode 'org-mode)
      (org-babel-python-session-buffer)
      (org-babel-switch-session "*Python*")
      (setq org-babel-python-command (concat
                                      (getenv "VIRTUAL_ENV") "/bin/python"))))

  (add-hook 'after-save-hook 'reset-python-session)


#+end_src

#+RESULTS:
| reset-python-session | efs/org-babel-tangle-config | rmail-after-save-hook | doom-modeline-update-vcs-text | doom-modeline-update-vcs-icon | doom-modeline-update-buffer-file-name |

** bindings
:LOGBOOK:
CLOCK: [2023-07-26 mié 14:36]
:END:
#+begin_src emacs-lisp
...
#+RESULTS:
| org-superstar-mode | dw/org-mode-setup | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-fold-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes | (closure ((mode . org-mode-hook) (--dolist-tail--) bootstrap-version t) nil (display-line-numbers-mode 0)) |
...
#+RESULTS:
| org-make-toc-mode | org-tempo-setup | org-superstar-mode | org-babel-jupyter-make-local-aliases | jupyter-org-interaction-mode | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | dw/org-mode-setup | ob-ipython-auto-configure-kernels | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes | (closure ((mode . org-mode-hook) (--dolist-tail--) bootstrap-version t) nil (display-line-numbers-mode 0)) |
...
#+RESULTS:
| (lambda nil (setq fill-column 88) (auto-fill-mode t) (display-fill-column-indicator-mode 1)) | (lambda nil (setq fill-column 88) (auto-fill-mode t) (display-fill-column-indicator-mode nil)) | rainbow-mode | (closure (bootstrap-version t) nil (setq fill-column 88) (auto-fill-mode t) (display-fill-column-indicator-mode 1)) | blacken-mode | tree-sitter-hl-mode | tree-sitter-mode | (closure (bootstrap-version t) nil (conda-env-activate-for-buffer)) | eglot-ensure | evil-collection-python-set-evil-shift-width | doom-modeline-env-setup-python |
*** Black
#+begin_src emacs-lisp
...
#+RESULTS:
| org-make-toc | py-isort-before-save |
